...
// Componente para renderizar ícone personalizado ou Octicon
...
export default function Activity() {
  ...
  useEffect(() => {
    const fetchAtividades = async () => {
      try {
        ...
        // Transformar dados para formato correto do SectionList
        ...
        setIsOnline(false);
        // Dados completos de fallback baseados no db.json
        setAtividades([
          {
            title: "2024-09-20",
            data: [
  ...
  const iconColor = (iconName) => {
    if (iconName === "trophy") return "#CDA34F"; // dourado - conquistas
    if (iconName === "ecocoin") return "#F5A623"; // dourado para EcoCoin personalizado
    if (iconName === "dependabot") return "#4CAF50"; // verde - ganho ecoCoins (backup)
    if (iconName === "dash") return colors.error; // vermelho - perda ecoCoins
    if (iconName === "plus-circle") return colors.success; // verde - ganhos gerais
    if (iconName === "x-circle") return colors.error; // vermelho - perdas gerais
    if (iconName === "location") return "#2196F3"; // azul - novos percursos
    if (iconName === "alert") return "#FF5722"; // laranja/vermelho - alertas
    if (iconName === "sync") return "#9C27B0"; // roxo - otimizações
    if (iconName === "graph") return "#FF9800"; // laranja - regressões
    if (iconName === "checklist") return colors.primary; // verde tema - metas
